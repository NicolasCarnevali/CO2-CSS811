...
void loop() {
  ...
  float TVOC_0, TVOC_1, TVOC_2;
  float TVOC_0_old, TVOC_1_old, TVOC_2_old;
  float moy_0, moy_1, moy_2;
  ...
  if (ens160.update() == RESULT_OK)
     {
        if (ens160.hasNewData())
        {
          if (MyCurrentPort == 0)
          {
            TVOC_0 = ens160.getTvoc();
            moy_0 = (moy_0 + TVOC_0)/2;
          }
          if (MyCurrentPort == 1)
          {
            TVOC_1 = ens160.getTvoc();
            moy_1 = (moy_1 + TVOC_1)/2;
          }
          if (MyCurrentPort == 2)
          {
            TVOC_2 = ens160.getTvoc();
            moy_2 = (moy_2 + TVOC_2)/2;
          }
